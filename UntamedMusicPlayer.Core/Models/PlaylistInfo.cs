...
{
    private static readonly ILogger _logger = CoreLoggingService.CreateLogger<PlaylistInfo>();
    ...
    /// <returns></returns>
    public async Task AddSongs(List<IBriefSongInfoBase> songs, CloudMusicApiService cloudApi)
    {
        foreach (var song in songs)
        {
            var coverPathIndex = IsCoverEdited ? -1 : await TryAddCoverPath(song, cloudApi);
    ...
    /// 向播放列表中添加一首歌曲
    /// </summary>
    public async Task Add(IBriefSongInfoBase song, CloudMusicApiService cloudApi)
    {
        var coverPathIndex = IsCoverEdited ? -1 : await TryAddCoverPath(song, cloudApi);
        var indexedSong = new IndexedPlaylistSong(SongList.Count, song, coverPathIndex);
        SongList.Add(indexedSong);
    ...
    /// 向播放列表中添加多首歌曲
    /// </summary>
    public async Task AddRange(IEnumerable<IBriefSongInfoBase> songs, CloudMusicApiService cloudApi)
    {
        foreach (var song in songs)
        {
            var coverPathIndex = IsCoverEdited ? -1 : await TryAddCoverPath(song, cloudApi);
    ...
    /// 从播放列表中删除一首歌曲
    /// </summary>
    public async Task Delete(IndexedPlaylistSong song, CloudMusicApiService cloudApi)
    {
        ...
        if (needRefillCover)
        {
            ...
            {
                await RefillCoverPaths(cloudApi);
            }
        }
    }
    ...
    /// <returns></returns>
    private async Task<int> TryAddCoverPath(IBriefSongInfoBase song, CloudMusicApiService cloudApi)
    {
        if (CoverPaths.Count >= 4)
        {
            return -1;
        }
        var coverPath = await CloudMusicModelFactory.GetCoverPathAsync(song, cloudApi);
    ...
    /// </summary>
    private async Task RefillCoverPaths(CloudMusicApiService cloudApi)
    {
        foreach (var indexedSong in SongList)
        {
            ...
            {
                var coverPath = await CloudMusicModelFactory.GetCoverPathAsync(
                    indexedSong.Song,
                    cloudApi
                );
                if (!string.IsNullOrWhiteSpace(coverPath))

...
{
    private static readonly ILogger _logger = CoreLoggingService.CreateLogger(nameof(FileManager));

    /// <summary>
    /// 保存音乐库数据到文件
    /// </summary>
    public static async Task SaveLibraryDataAsync(
    ...
    )
    {
        ...
        try
        {
            ...
            var songsTask = SaveObjectToFileAsync(libraryFolder, "Songs", songs);
            ...
            await Task.WhenAll(fingerprintTask, songsTask, musicFoldersTask);
        }
        catch (Exception ex)
        {
    ...
    public static async Task SavePlaylistDataToM3u8Async(IEnumerable<PlaylistInfo> playlists)
    {
        ...
        await Task.Run(async () =>
        {
            try
            {
                ...
                await M3u8Helper.ExportPlaylistsToM3u8Async(playlistFolder.Path, playlists);
            }
            catch (Exception ex)
            {
    ...
    )
    {
        ...
        try
        {
            ...
            await Task.WhenAll(songsTask, musicFoldersTask);

            var songsBytes = songsTask.Result;
            var musicFoldersBytes = musicFoldersTask.Result;

            if (songsBytes is null || musicFoldersBytes is null)
            {
                return (true, data);
            }
            ...
            if (songsList is null || musicFoldersDict is null)
            {
                return (true, data);
            }
    ...
    )
    {
        try
        {
            ...
            {
                return await LoadPlaylistDataFromM3u8Async(cloudApi);
            }
    ...
    )
    {
        try
        {
            ...
            foreach (var file in files)
            {
                ...
                await playlist.AddRange(songs, cloudApi);
...
public sealed class MusicLibraryData
{
    ...
    public ConcurrentDictionary<string, byte> MusicFolders { get; set; } = [];
    ...
    {
        Songs = new ConcurrentBag<BriefLocalSongInfo>(songs);

...
public static class ZLoggerExtensions
{
    extension(ILogger logger)
    {
        ...
        public void UserAction(string actionName) =>
        ...
        public void EditingSongInfoOther(
                    string title,
            Exception exception
        ) =>
            logger.ZLogError(
        ...
        // 高性能性能监控日志
        public void LogPerformanceStart(
                    string operationName,
            int operationId
        ) => logger.ZLogDebug($"[{operationId:X8}] 开始执行: {operationName}");

        public void LogPerformanceEnd(
                    string operationName,
            int operationId,
            double elapsedMs
        ) =>
        ...
        public void AudioStreamCreated(
                    string filePath,
            int streamHandle,
        ...
        public void PlaybackBufferUnderrun(
                    string songTitle,
            int bufferLevel
        ) => logger.ZLogWarning($"播放缓冲区不足: {songTitle}, 缓冲区水平: {bufferLevel}%");

        // 网络请求日志
        public void HttpRequestStarted(
                    string method,
            string url,
            int requestId
        ...
        // 文件操作日志
        public void FileOperationStarted(
                    string operation,
            string filePath,
            int operationId
        ) => logger.ZLogDebug($"[{operationId:X8}] 文件操作开始: {operation} -> {filePath}");

        public void FileOperationCompleted(
                    int operationId,
            double elapsedMs,
            long? fileSize = null
        )
        ...
        // 数据库操作日志
        public void DatabaseOperationStarted(
                    string operation,
            int operationId
